					  ...
					  const char *location)
{
	...
	poll_event_wake_pollthread(poll_ev);
    #else
    printf("FIXME poll_event_schedule_immediate is not implemnted!\n");
    #endif
}
...
#else
    printf("ERROR poll_event_set_fd_flags\n");
#endif
...
#else
            printf("ERROR in poll_event_sync_arrays\n");
#endif
		}
...
#else
    printf("ERROR in poll event loop poll!\n");
#endif
...
#else
    printf("ERROR poll_event_loop_once\n");
#endif
...
static const struct tevent_ops poll_event_ops = {
	/*.context_init		=*/ poll_event_context_init,
	/*.add_fd			=*/ poll_event_add_fd,
	/*.set_fd_close_fn	=*/ NULL,//TODO tevent_common_fd_set_close_fn,
