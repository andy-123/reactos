...
#else
    memset(&dbgc_config[ndx], 0, sizeof(dbgc_config[ndx]));
    dbgc_config[ndx].fd = -1;
...
void debug_setup_talloc_log(void)
{
    ...
    #else
    D_ERR("FIXME debug_setup_talloc_log\n");
    #endif
}
...
bool dbghdrclass(int level, int cls, const char *location, const char *func)
{
    ...
    #else
    /* no time atm */
	hs_len = snprintf(header_str, sizeof(header_str), "[%2d",
			  level);
    #endif
...
#else
    msgbuf = malloc(2048);
    res = vsprintf(msgbuf, format_str, ap);
#endif
	if (res != -1) {
