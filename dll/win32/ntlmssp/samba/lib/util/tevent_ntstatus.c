...
#else
    /* problem ... status is LONG ... and value is negative
     * or (|) will set higher 32 bit to 0xffffffff.
     * maybe in native samba build NTSTATUS isn't a LONG */
	err = ((uint64_t)TEVENT_NTERROR_MAGIC << 32) | (uint32_t)status;
#endif
