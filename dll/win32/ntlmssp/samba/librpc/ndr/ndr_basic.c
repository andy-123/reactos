	...
	*v = (uint32_t)v64;
	if (unlikely(v64 != *v)) {
		...
		return ndr_pull_error(ndr, NDR_ERR_NDR64, __location__ ": non-zero upper 32 bits 0x%016llx\n",
			 (unsigned long long)v64);
#else
        /* getting compiler error with code above (gcc) */
		DEBUG(0,(__location__ ": non-zero upper 32 bits 0x%"PRIu64"\n",
			 (unsigned long long)v64));
		return ndr_pull_error(ndr, NDR_ERR_NDR64, __location__ ": non-zero upper 32 bits 0x%"PRIu64"\n",
			 (unsigned long long)v64);
#endif
	}
	return err;
